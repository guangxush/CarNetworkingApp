...
import com.example.chelianwang.overlayutil.DrivingRouteOverlay;
    ...
    private void initView() {
       ...
       switch (s) {
           ...
           mSearch.drivingSearch(new DrivingRoutePlanOption().from(st).to(en));
               break;
           case 2:
               String qishi=intent.getStringExtra("qishi");
               String mudi=intent.getStringExtra("mudi");
               PlanNode start=PlanNode.withCityNameAndPlaceName("北京",qishi);
               PlanNode end=PlanNode.withCityNameAndPlaceName("北京",mudi);
               mSearch.drivingSearch(new DrivingRoutePlanOption().from(start).to(end));
               break;
           case 0:
               Toast.makeText(this,"无导航信息",Toast.LENGTH_SHORT);
               break;
           default:
               break;
